...
<body>
	...
	<div class="expert_doctors_area">
		<div class="container">
			...
			<div class="row justify-content-center">
				<%
				List<Donor> donors = (List) session.getAttribute("donor");
				for (Donor donor : donors) {
				%>
				<div class="col-lg-4 col-md-6">
					<div class="single_expert">
						<div class="expert_thumb">
							<img src="<%=donor.getMedimage()%>" alt="" height="200px">
						</div>
						<br>
						<div class="expert_thumb">
							<img src="<%=donor.getPreimage()%>" alt="" height="200px">
						</div>
						<br>
						<div class="expert_thumb">
							<img src="<%=donor.getAadharimage()%>" alt="" height="200px">
						</div>
						<div class="experts_name text-left">
							<h3><%=donor.getMedname()%></h3>
							<span>Quantity of medicines:</span> <span><%=donor.getQuantity()%></span><br>
							<span>Donor Name:</span> <span><%=donor.getName()%></span><br>
							<span>Phone No:</span> <span><%=donor.getPhone()%></span><br>
							<span>Address:</span> <span><%=donor.getAddress()%></span><br>
							<span>Email:</span> <span><%=donor.getEmail()%></span><br> <a
								href="upload?id=<%=donor.getId()%>&medimage=<%=donor.getMedimage()%>&preimage=<%=donor.getPreimage()%>&aadharimage=<%=donor.getAadharimage()%>&medname=<%=donor.getMedname()%>&quantity=<%=donor.getQuantity()%>&name=<%=donor.getName()%>&phone=<%=donor.getPhone()%>&address=<%=donor.getAddress()%>&email=<%=donor.getEmail()%>"
								class="boxed-btn6">Upload</a> <a
								href="delete?id=<%=donor.getId()%>" class="boxed-btn6">Delete</a>
						</div>
					</div>
				</div>
				<%
